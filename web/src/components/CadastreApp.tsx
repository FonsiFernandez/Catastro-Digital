...
export default function CadastreApp() {
  ...
  useEffect(() => {
    if (selectedRc && !selectedParcel && !data.loading) setSelectedRc(null);
  ...
  useEffect(() => {
    ...
    const selectedTarget = selectedParcel && !selectedParcel.properties.is_deleted
    ...
    const timer = window.setTimeout(() => {
      void data
        ...
        .catch((error) => {
          if (fieldRequestIdRef.current !== requestId) return;
          setFieldDataError(readableApiError(error, "No se pudo calcular tu posición respecto a la finca"));
        });
    }, 500);
  ...
  const searchParcel = async () => {
    ...
    try {
      ...
      data.setNotice({
        type: "error",
        message: readableApiError(error, "No se pudo consultar la referencia catastral"),
      });
    } finally {
  ...
  const startFieldMode = () => {
    ...
    if (baseMap === "street") setBaseMap("aerial");
    field.start();
    data.clearNotice();
  };

  const startIdentifyMode = () => {
    stopFieldMode();
    identifyRequestIdRef.current += 1;
  ...
  const identifyPoint = async (longitude: number, latitude: number) => {
    if (!identifyMode || identifyLoading || savingPreview) return;
    ...
    try {
      const result = await data.identifyParcel(longitude, latitude);
      ...
      data.setNotice({
        type: "error",
        message: readableApiError(error, "No se pudo identificar la parcela bajo el cursor"),
      });
    } finally {
  ...
  const savePreviewParcel = async () => {
    if (!previewParcel || identifyLoading || savingPreview) return;

    setSavingPreview(true);
    try {
      const saved = await data.lookupParcel(previewParcel.properties.cadastral_ref);
      ...
      data.setNotice({
        type: "error",
        message: readableApiError(error, "No se pudo guardar la parcela seleccionada"),
      });
    } finally {
  ...
  const openSavedPreview = async () => {
    ...
    try {
      if (previewParcel.properties.is_deleted) {
        await data.updateParcel(previewParcel.properties.cadastral_ref, { is_deleted: false });
      }
  ...
  const updateSelectedParcel = async (update: Parameters<typeof data.updateParcel>[1]) => {
    if (!selectedParcel) return;
    await data.updateParcel(selectedParcel.properties.cadastral_ref, update);
  };

  const deleteSelectedParcel = async () => {
    if (!selectedParcel) return;
    await data.deleteParcel(selectedParcel.properties.cadastral_ref);
    if (!includeDeleted) setSelectedRc(null);
  };
  ...
  return (
    <main className={fieldMode ? "cad-app field-mode-active" : "cad-app"}>
      <Sidebar
        ...
        onCenterSelected={() => {
          if (selectedParcel) mapRef.current?.fitParcel(selectedParcel);
        }}
      ...
      <section className={identifyMode ? "cad-map-shell is-identifying" : "cad-map-shell"}>
        <CadastreMap
          ...
          onSelectParcel={setSelectedRc}
          onIdentifyPoint={(longitude, latitude) => void identifyPoint(longitude, latitude)}
        />
        ...
        <div className="map-floating-bar">
          <div className="map-status">
            ...
            <span>
              {visibleParcels.length} {visibleParcels.length === 1 ? "parcela" : "parcelas"}
              {totalVisibleAreaHa > 0 ? ` · ${formatHectares(totalVisibleAreaHa)}` : ""}
            </span>
          </div>
          <button
            type="button"
            className="map-action"
            disabled={!visibleParcels.length || identifyMode || fieldMode}
            onClick={() => mapRef.current?.fitAll()}
          >
            <CrosshairIcon /> Ver todas
          </button>
          {!identifyMode && !fieldMode ? (
            <ParcelPickerPanel
              active={false}
              loading={false}
              preview={null}
              alreadySaved={false}
              onStart={startIdentifyMode}
              onCancel={cancelIdentifyMode}
          ...
          {!identifyMode && !fieldMode ? (
            <FieldModePanel
              ...
              lockedToSelection={Boolean(selectedParcel && !selectedParcel.properties.is_deleted)}
              onStart={startFieldMode}
              onStop={stopFieldMode}
              onCenterUser={() => {
                if (field.location) mapRef.current?.centerLocation(field.location);
              }}
              onCenterParcel={() => {
                if (fieldTarget) mapRef.current?.fitParcel(fieldTarget.parcel);
              }}
              onUseDetectedParcel={() => {
                if (fieldTarget) setSelectedRc(fieldTarget.parcel.properties.cadastral_ref);
              }}
      ...
      <Notice notice={data.notice} onClose={data.clearNotice} />
    </main>
  );
}
